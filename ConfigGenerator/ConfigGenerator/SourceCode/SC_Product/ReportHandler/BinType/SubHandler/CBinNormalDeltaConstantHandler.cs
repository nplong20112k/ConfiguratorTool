...
{
    class CBinNormalDeltaConstantHandler : ABinSubHandler
    {
        ...
        private const string BIN_REPORT_NORMAL_CONSTANT_BINARY_FORMAT_VALUE_FILE_NAME   = "ciro_ndcf.bin";
        private const string MAGIC_NUMBER_NAME                                          = "NDCF";
        ...
        public CBinNormalDeltaConstantHandler() : base()
        {
            m_FileProcessor = CFactoryBinFileProcessor.GetInstance().GetFileProcessor(BIN_FILE_PROCESSOR_TYPE.BIN_NORMAL_DELTA_CONSTANT);
                ...
                if (oDataIn != null)
                {
                    ...
                    if (DeltaTableObj != null)
                    {
                        ...
                        foreach (DELTA_TABLE_ITEM element in lsDeltaTableGroup)
                        {
                            ...
                            if (sSizeValue == "1")
                            {
                                sValue = element.m_sValue.PadLeft(2, '0'); 
        ...
        private bool AddDataRecord(string sInterfaceType, string sTagName, string sDefaultValue)
        {
            bool bRet = false;
            if(m_lsNormalEnumInfo != null && m_lsNormalEnumInfo.Count > 0)
            {
                ...
                foreach (INTERFACE_VALID element in m_slsInterfaceValid)
                {
                    if(element.m_sInterfaceName == sInterfaceType)
                    {
                        ...
                        lsByteArrayElement.Add(CStringByteConvertor.convertHexStringToByteArray(sDefaultValue));
                        break;
                    }
                }
